...
async function migrateCountries(pgClient, sqliteDb) {
  ...
  const result = await pgClient.query('SELECT id, name FROM countries ORDER BY id;');
  
  const insert = sqliteDb.prepare('INSERT INTO countries (id, name) VALUES (?, ?)');
...
async function migrateCities(pgClient, sqliteDb) {
  ...
  const insertMany = sqliteDb.transaction((cities) => {
    for (const city of cities) {
      const embeddingBlob = city.embedding ? vectorToBlob(city.embedding) : null;
      insert.run(
        city.id,
        city.name,
        city.lat,
...
function verifyMigration(db) {
  ...
  const tables = ['countries', 'cities', 'categories', 'places', 'place_categories'];
