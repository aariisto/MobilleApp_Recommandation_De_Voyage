...
def generate_categories_gpt(city_name: str, categories: List[str]) -> str:
    ...
    nature_leaf = _extract_leaf_values(categories, "natural")
    nature_leaf = [_humanize_token(x) for x in nature_leaf]
    nature_leaf = _dedupe_keep_order(nature_leaf)[:3]
    ...
    # --- Fun / Sport ---
    has_nightclub = has_exact("adult.nightclub")
    has_casino = has_exact("adult.casino")
    has_theme_park = has_exact("entertainment.theme_park")
    has_ski = has_prefix("ski")
    has_stadium = has_exact("sport.stadium")
    ...
    if has_nature:
        if nature_leaf:
            chunks.append(f"beautiful landscapes like {_join_natural(nature_leaf)}")
        else:
    ...
    if not chunks:
        return f"{city_name} offers a mix of travel experiences and local atmosphere."

    # Keep the sentence concise and focused
    chunks = chunks[:3]
    return f"{city_name} is a great choice for travelers seeking {_join_natural(chunks)}."
